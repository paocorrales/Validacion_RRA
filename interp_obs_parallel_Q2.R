# Script para interpolar el pronóstico a las observaciones en el caso de la humedad
...
system.time(out <- foreach(f = 1:length(files),
  ...
  fcst <- ReadNetCDF(files[f], vars = c("XLONG", "XLAT", "Q2", "T2", "PSFC"), 
                     subset = list(lat = 5:145, lon = 5:95))
  
  fcst[, RH := rh(T2, Q2/1000, PSFC*100)*100] %>% 
    .[, Q2 := NULL] %>% 
    .[, T2 := NULL] %>% 
    .[, PSFC := NULL]
  ...
  # Interpolo
  temp <- fcst[, c(interp(XLONG, XLAT, RH, output = "points", 
