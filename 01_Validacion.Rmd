...
```{r read_obs}
filepath <- '../obs_RRA/20181120_00/obs_20181120_00_asimiladas.dat'
...
```{r}
obs2 <- read.obs.asim('../obs_RRA/20181120_01/obs_20181120_01_asimiladas.dat')
...
obs <- read.obs.asim("../obs_RRA/20181120_1*/*s.dat", keep.time.slot = c(1, 7)) 
...
Está dando al revez que lo que pensaba. Pruebo con otro par de tiempos.

```{r}
#Slots 1 y 7 de 20181120  a las 00 y las 01 horas
obs <- read.obs.asim("../obs_RRA/20181120_0?/*s.dat", keep.time.slot = c(1, 7)) 

#Quiero el slot 7 del archivo de las 00UTC y el slot 1 del archivo de las 01 UTC. Ambos corresponden a las 00:00
obs <- obs[time.obs == as_datetime("2018-11-20 00:00:00")]

obs[, .N, keyby = .(time.slot)] %>% 
  knitr::kable()
```
...
fcst[time.obs == as_datetime("2018-11-22T00:00:00Z")] %>% 
  ...
  scale_color_divergent(name = "Diferencia de \nla media del ensamble", limits = c(-7, 7)) +
  coord_fixed() +
  theme(legend.position="bottom")
...
Y mirando la relación entre las observaciones y el pronóstico se ve que si bien hay una relación directa y la mayoría d elos puntos se ubican cerca de la recta hay una región donde se ve que el pronóstico sobre estima a las observaciones y algunos puntos anómalos que podrían corresponde a estaciones con problemas sistemáticos.
