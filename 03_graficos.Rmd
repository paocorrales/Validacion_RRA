...
```{r}
copy(rmse) %>% 
  .[, list(mean.rmse = mean(rmse)), by = .(verif, fecha.ini)] %>% 
  ggplot(aes(verif, mean.rmse)) +
  geom_line(aes(group = interaction(fecha.ini), color = fecha.ini), alpha = 1, size = 0.1) +
  # scale_colour_viridis_c(trans = _trans()) +
  labs(title = "RMSE de cada ciclo de pronóstico (media del emsable)",
       x = "Plazo de verificación")
```
...
copy(rmse) %>% 
  .[, list(mean.rmse = mean(rmse), sd.rmse = sd(rmse)), by = .(hour(fecha.verif))] %>% 
