...
estadisticos[, var.type := case_when(var %in% c(2819, 82819) ~ "u",
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  ...
  scale_x_continuous("Forecast Time (hours)") +
  scale_y_continuous(name = NULL) +
  facet_grid(var.type ~ hora.ini, scales = "free_y") +
  theme_minimal()
```
...
est_espacial[, .(rmse.mean = mean(rmse, na.rm = TRUE),
                 bias.mean = mean(bias, na.rm = TRUE)), by = .(verif, lon, lat)] %>% 
  .[verif %in% c(0, 6, 12, 18, 24) & rmse.mean < 10] %>% 
  na.omit() %>% 
  ggplot(aes(ConvertLongitude(lon), lat)) +
  geom_point(aes(color = rmse.mean, size = rmse.mean)) +
...
est_espacial[, .(rmse.mean = mean(rmse, na.rm = TRUE),
                 bias.mean = mean(bias, na.rm = TRUE)), by = .(var.type, verif, lon, lat)] %>% 
  .[verif %in% c(0, 6, 12, 18) & rmse.mean < 5 & var.type != "rh"] %>% 
  na.omit() %>% 
  ggplot(aes(ConvertLongitude(lon), lat)) +
