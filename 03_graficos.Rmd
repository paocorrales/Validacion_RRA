...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  ...
  theme(strip.background = element_rect(fill = "white"),
        ...
        legend.background = element_rect(color = "darkgrey", fill = NA, size = 0.5),
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  ...
  ggplot(aes(verif, value)) +
   geom_line(data = ~.x[fisica != "MEAN"], 
                       aes(color = fisica, group = interaction(estadistico, fisica))) +
    geom_line(data = ~.x[fisica == "MEAN"], 
                       aes(group = interaction(estadistico, fisica)),  color = "black") +
  scale_alpha_discrete(guide = "none", range = c(0.05, 1)) +
...
ggsave("fig/parametrizaciones.pdf")
```
