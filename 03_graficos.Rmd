...
estadisticos <- estadisticos %>% 
  .[, .(rmse = mean(rmse),
        bias = mean(bias)), by = .(verif, fecha.ini, var, fisica)] %>%
  .[, ":="(ens = 0,
           fisica = "MEAN")] %>% 
  rbind(estadisticos) %>% 
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  .[var.type != "rh"] %>% 
  .[hora.ini %in% c(0, 6, 12, 18), .(value = mean(value)), by = .(var.type, hora.ini, verif, ens, estadistico)] %>%
  ...
  scale_color_grey(name = NULL, start = 0, end = 0.5) +
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  ...
  .[var.type != "rh" & micro %in% c("MEAN", "BMJ")] %>% 
  .[hora.ini %in% c(0, 6, 12, 18), .(value = mean(value)), 
    by = .(var.type, hora.ini, verif, pbl, estadistico)] %>%
  # .[estadistico == "rmse"] %>% 
  ggplot(aes(verif, value)) +
  geom_line(data = ~.x[pbl == "MEAN"],
            aes(group = interaction(estadistico, pbl)),  
            color = "grey20",
            size = 0.5) +
  geom_line(data = ~.x[pbl != "MEAN"], 
            aes(color = pbl, linetype = estadistico,
                group = interaction(estadistico, pbl))) +
  ...
  facet_grid(var.type ~ hora.ini, scales = "free_y", switch = "y", 
             labeller = labeller(hora.ini = c("0" = "00 UTC", "6" = "06 UTC",
                                              "12" = "12 UTC", "18" = "18 UTC"))) +
  labs(subtitle = "Microphysics = BMJ") +
...
# estadisticos[, hora.ini := hour(fecha.ini)] %>% 
#   .[var.type != "rh"] %>% 
#   .[hora.ini %in% c(0), .(value = mean(value)), 
#     by = .(var.type, hora.ini, verif, ens, estadistico, fisica)] %>%
#   # .[estadistico == "rmse"] %>% 
#   ggplot(aes(verif, value)) +
#   geom_line(data = ~.x[fisica != "MEAN"], 
#                        aes(color = fisica, group = interaction(estadistico, ens))) +
#     geom_line(data = ~.x[fisica == "MEAN"], 
#                        aes(group = interaction(estadistico, ens)),  color = "black") +
#   scale_alpha_discrete(guide = "none", range = c(0.05, 1)) +
#   scale_color_brewer(palette = "Set1") +
#   geom_hline(yintercept = 0, color = "darkgray") +
#   scale_x_continuous("Forecast lead time (hrs)") +
#   scale_y_continuous(name = NULL, position = "right") +
#   labs(title = "Init time = 00Z") +
#   facet_wrap(~var.type, scales = "free_y", switch = "y", 
#              ncol = 2,
#              labeller = labeller(hora.ini = c("0" = "00 UTC", "6" = "06 UTC",
#                                               "12" = "12 UTC", "18" = "18 UTC"))) +
#   theme_linedraw(base_size = 16) +
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  tidyr::separate(fisica, into = c("micro", "pbl")) %>% 
  .[, pbl := if_else(is.na(pbl), "MEAN", pbl)] %>% 
  .[var.type != "rh" & pbl %in% c("MEAN", "MYNN2")] %>% 
  .[hora.ini %in% c(0, 6, 12, 18), .(value = mean(value)), 
    by = .(var.type, hora.ini, verif, micro, estadistico)] %>%
  # .[estadistico == "rmse"] %>% 
  ggplot(aes(verif, value)) +
  geom_line(data = ~.x[micro == "MEAN"],
            aes(group = interaction(estadistico, micro)),  
            color = "grey20",
            size = 0.5) +
  geom_line(data = ~.x[micro != "MEAN"], 
            aes(color = micro, linetype = estadistico,
                group = interaction(estadistico, micro))) +
  ...
  facet_grid(var.type ~ hora.ini, scales = "free_y", switch = "y", 
             labeller = labeller(hora.ini = c("0" = "00 UTC", "6" = "06 UTC",
                                              "12" = "12 UTC", "18" = "18 UTC"))) +
  labs(subtitle = "PBL = MYNN2") +
  theme_linedraw(base_size = 16) +
  theme(strip.background = element_rect(fill = "white"),
        strip.text = element_text(color = "black"),
        panel.grid = element_line(color = "grey80"))

ggsave("fig/mynn2.pdf", width = 20, height = 15, units = "cm")
```
...
est_espacial[, .(rmse.mean = mean(rmse, na.rm = TRUE),
  ...
  ggplot(aes(ConvertLongitude(lon), lat)) +
  geom_point(data = function(x) x[variable == "rmse.mean"], aes(color = value), size = 2) +
  ...
  geom_point(data = function(x) x[variable == "bias.mean"], aes(color = value, shape = factor(sign(value))),
             size = 2) +
