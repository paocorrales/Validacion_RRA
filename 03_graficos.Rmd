...
ggsave("fig/observaciones.eps", dpi = 300, width = 20, units = "cm")
```
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  ...
  theme(strip.background = element_rect(fill = "white"),
        strip.text = element_text(color = "black"), panel.grid = element_line(color = "grey80"),
        legend.position = c(.98, .15),
        legend.box.background = element_rect(color = "darkgrey", fill = NA, size = 1),
        legend.justification = c("right", "top"),
        legend.margin = margin(0, 0, 0, 0), legend.spacing.y = unit(0.01, "cm"))

ggsave("fig/rmse_bias.pdf", dpi = 300, width = 22, height = 15, units = "cm")

estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  .[var.type != "rh"] %>% 
  .[hora.ini %in% c(0, 6, 12, 18), .(value = mean(value)), by = .(var.type, hora.ini, verif, fisica, estadistico)] %>%
  # .[estadistico == "rmse"] %>% 
  ggplot(aes(verif, value)) +
  geom_line(aes(color = fisica, group = interaction(estadistico, fisica))) +
  scale_alpha_discrete(guide = "none", range = c(0.05, 1)) +
  # scale_color_grey(name = NULL, start = 0, end = 0.5) +
  geom_hline(yintercept = 0, color = "darkgray") +
  scale_x_continuous("Forecast lead time (hrs)") +
...
estadisticos[, hora.ini := hour(fecha.ini)] %>% 
  ...
  # .[estadistico == "rmse"] %>% 
  ggplot(aes(verif, value)) +
  geom_line(aes(color = fisica, group = interaction(estadistico, ens))) +
  scale_alpha_discrete(guide = "none", range = c(0.05, 1)) +
  # scale_color_grey(name = NULL, start = 0, end = 0.5) +
  geom_hline(yintercept = 0, color = "darkgray") +
  scale_x_continuous("Forecast lead time (hrs)") +
